...
typedef enum
{
    ...
    LINE_PARAM    = (1u << 6), // Hexen-style param actions
    LINE_PASSUSE  = (1u << 7), // Boom's "Pass Use Through" line flag
    LINE_BLOCK    = (1u << 8), // MBF21's entity blocking flag
    LINE_3DMIDTEX = (1u << 9), // EE's 3D middle texture
    LINE_ALPHA    = (1u << 10), // opacity percentage
    LINE_TRANMAP  = (1u << 11), // ditto, also customizable LUT

    SIDE_OFFSET   = (1u << 12), // texture X/Y alignment
    SIDE_SCROLL   = (1u << 13), // texture scrolling property
    SIDE_LIGHT    = (1u << 14), // independent light levels

    SEC_ANGLE     = (1u << 15), // plane rotation
    SEC_OFFSET    = (1u << 16), // plane X/Y alignment
    SEC_EE_SCROLL = (1u << 17), // EE's original plane scrolling property
    SEC_SCROLL    = (1u << 18), // DSDA's later plane scrolling property
    SEC_LIGHT     = (1u << 19), // independent light levels

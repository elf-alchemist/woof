...
// from SLADE.
//
patch_t *V_LinearToTransPatchTag(const byte *data, int width, int height,
...
static patch_t *DummyPatchTag(int lump, pu_tag tag)
{
...
static void *DummyFlatTag(int lump, pu_tag tag)
{
...
static patch_t *DummyPatch(int lump)
{
    int num = (W_CheckNumForName)("TNT1A0", ns_sprites);
    int len = W_LumpLength(num);
...
patch_t *V_CachePatchNumTag(int lump, pu_tag tag)
{
    ...
    patch_t *patch = V_LinearToTransPatchTag(png.image, png.width, png.height,
...
error:
    FreePNG(&png);
    Z_Free(buffer);
    return DummyFlatTag(lump, tag);
}

patch_t *V_CachePatchNum(int lump)
{
    ...
    void *buffer = W_CacheLumpNum(lump);
    int buffer_length = W_LumpLength(lump);

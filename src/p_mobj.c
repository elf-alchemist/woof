...
//
// P_SpawnMapThing_Hexen
//

void P_SpawnMapThing_Hexen(mapthing_hexen_t* hmthing)
{
  int    i;
  ...
  if (demo_compatibility || 
      (demo_version >= DV_MBF && hmthing->options & MTF_RESERVED))
    hmthing->options &= MTF_EASY|MTF_NORMAL|MTF_HARD|MTF_AMBUSH|MTF_NOTSINGLE;
  ...
  if (hmthing->type == 11)
    {
      ...
      size_t offset = hexen_deathmatch_p - hexen_deathmatchstarts;
      ...
      if (offset >= num_deathmatchstarts)
	{
	  num_deathmatchstarts = num_deathmatchstarts ?
	    num_deathmatchstarts*2 : 16;
	  hexen_deathmatchstarts = Z_Realloc(hexen_deathmatchstarts,
				     num_deathmatchstarts *
				     sizeof(*hexen_deathmatchstarts),
				     PU_STATIC, 0);
	  hexen_deathmatch_p = hexen_deathmatchstarts + offset;
	}
      memcpy(hexen_deathmatch_p++, hmthing, sizeof*hmthing);
      return;
    }

  // check for players specially

  if (hmthing->type <= 4 && hmthing->type > 0) // killough 2/26/98 -- fix crashes
    {
      // killough 7/19/98: Marine's best friend :)
      if (!netgame && hmthing->type > 1 && hmthing->type <= dogs+1 &&
	  !players[hmthing->type-1].secretcount)
	{  // use secretcount to avoid multiple dogs in case of multiple starts
	  players[hmthing->type-1].secretcount = 1;

	  // killough 10/98: force it to be a friend
	  hmthing->options |= HMTF_FRIEND;
	  i = MT_DOGS;
	  goto spawnit;
	}

      // save spots for respawning in network games
      hexen_playerstarts[hmthing->type-1] = *hmthing;
      if (!deathmatch)
	P_SpawnPlayer_Hexen (hmthing);
  ...
  if (netgame == false)
    spawnMask = HMTF_GSINGLE;
  else if (deathmatch)
    spawnMask = HMTF_GDEATHMATCH;
  else
    spawnMask = HMTF_GCOOP;

  if (!(hmthing->options & spawnMask))
    return;
  ...
  if ((gameskill == sk_none && demo_compatibility) ||
    ...
    spawnMask = HMTF_NORMAL;

  if (!(hmthing->options & spawnMask))
    return;

  // [crispy] support MUSINFO lump (dynamic music changing)
  if (hmthing->type >= 14100 && hmthing->type <= 14164)
  {
      musid = hmthing->type - 14100;
      hmthing->type = mobjinfo[MT_MUSICSOURCE].doomednum;
  }

  // find which type to spawn

  // killough 8/23/98: use table for faster lookup
  i = P_FindDoomedNum(hmthing->type);
  ...
  if (i == num_mobj_types)
    {
      I_Printf(VB_WARNING, "P_SpawnMapThing_Hexen: Unknown Thing type %i at (%i, %i)",
	      hmthing->type, hmthing->x, hmthing->y);
      return;
    }
...
spawnit:
  ...
  // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
  mobj = P_SpawnMobj (x,y,z, i);
  mobj->spawnpoint_hexen = *hmthing;

  mobj->tid     = hmthing->tid;
  mobj->special = hmthing->special;
  mobj->args[0] = hmthing->arg1;
  mobj->args[1] = hmthing->arg2;
  mobj->args[2] = hmthing->arg3;
  mobj->args[3] = hmthing->arg4;
  mobj->args[4] = hmthing->arg5;
  ...
  mobj->angle = (angle_t)ANG45 * (hmthing->angle/45);
  if (hmthing->options & MTF_AMBUSH)
    mobj->flags |= MF_AMBUSH;
